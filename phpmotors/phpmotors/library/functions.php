...
function buildClassificationList($classifications){ 
  ...
  function buildReviewList($reviewItems){
   ...
   foreach ($reviewItems as $reviewItem) {
      $date = date_create($reviewItem['reviewDate']);   
      $date = date_format($date,'m/d/Y');

      $nameF = substr($reviewItem['clientFirstname'], 0, 1);
      $reviewList .= '<li class="review-item">';
      $reviewList .= "<h2 class='review-name'>" . $nameF . "$reviewItem[clientLastname]" . " wrote " . " for the $reviewItem[invMake] $reviewItem[invModel]" .  " - $date </h2>";
      $reviewList .= "<p class='review-text'>$reviewItem[reviewText]</p>";
      $reviewList .= "<p><a href='/phpmotors/reviews?action=edit-review&reviewId=$reviewItem[reviewId]'>Edit</a>  <a href='/phpmotors/reviews?action=delete-review&reviewId=$reviewItem[reviewId]'>Delete</a></p>";
      $reviewList .= '</li>';
     }
  ...
  function buildReviewDisplay($reviews){
   ...
   $nameF = substr($review['clientFirstname'], 0, 1);
    ...
    $dv .= "<h2 class='review-name'>$nameF" . "$review[clientLastname]" . " wrote on " . $date . "</h2>";
